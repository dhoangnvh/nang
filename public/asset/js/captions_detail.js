...
$(document).ready(function() {
    ...
    $(document).on('click', '.btn-action-publish', function() {
        uploadCaption();
    })
    ...
    function saveCaption(status) {
        ...
        $.ajax({
            url: "captions/store/" + videoId + "/" + langId,
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
    ...
    function uploadCaption() {
        $('.sk-loading-full').removeClass('d-none');
        var videoId = $('.info-video-trans').data('video-id');
        ...
        $.ajax({
            ...
            error: function(data) {
                $('.sk-loading-full').addClass('d-none');
                skAlert('error', 'この動画はアプロード権限がありません。');
            }
        });
    };
    ...
    $(document).on('change', "#import-file", function() {
        if (this.files.length == 1) {
            ...
            formData.append("videoId", $('.info-video-trans').data('video-id'));
